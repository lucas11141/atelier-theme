...
jQuery( document ).ready(function($) {
	...
	if(!sessionStorage.getItem('site_toggle_links')) {
		sessionStorage.setItem('site_toggle_links', JSON.stringify({
			'website': '',
			'shop': ''
		}))
	}
	...
	$('.accordeon__item .accordeon__header').click(function(e) {
        ...
        $(".accordeon__item").find(".accordeon__content").not($accordion_content).parent(".accordeon__item").removeClass('accordeon__item--opened');
	...
	waitForElementToDisplay(".wpcf7-not-valid-tip",function(){
	...
	darkBackgrounds.forEach(element => {
		...
		backgroundElement.src = "https://atelier-delatron.de/wp-content/themes/atelier_theme/img/website/background_dark_image.jpg"
	...
	$('.woocommerce-product-gallery').each(function() {
		...
		// Init slider
		$(this).find('.woocommerce-product-gallery__wrapper').slick({
			autoplay: false,
			speed: 200,
			slidesToShow: 1,
			slidesToScroll: 1,
	...
	document.querySelectorAll('.woocommerce-shipping-methods li').forEach(element => {
		element.addEventListener('click', e => {
			...
			if(e.currentTarget.querySelector('.amount')) {
				amount = e.currentTarget.querySelector('.amount bdi').childNodes[0].nodeValue + '<span class="woocommerce-Price-currencySymbol">€</span>'
			} 
			if(amount === undefined) {
				amount = 'Kostenlos!'
			}
			...
			document.querySelector('.order-total bdi').innerHTML = `${total}<span class="woocommerce-Price-currencySymbol">€</span>`
